...
function parseCustomComponentWxss(full_path){
  return new Promise(function(resolve,reject){
    let prefix=full_path.replace(/\//g,'-').substring(0,full_path.length-5)
...
exports.parseCustomComponent=function  parseCustomComponent(page,full_path,key){
    ...
    util.parseImports(full_path, false, async (err, srcs) => {
      ...
      if(config&&config.usingComponents&&Object.keys(config.usingComponents).length>0){
        for(let key  in config.usingComponents){
          ...
          parseCustomComponent(page,fullpatht,key)
        }
      }
      
      g_pageComponents[page].push({
        c_tag:key,
        c_path:tag
      })
      ...
      pt.then(data=>{
        ...
        data[1]+="return $gwx(\'\./" + tag + ".wxml\')\
        }"
        var funcname=tag.replace(/\//g,'_')+"_genfunc"
        var head="window."+funcname+" = function (){"
        var content=head+data[1]
        ...
        util.createFile(distPath.path+'/'+path,fname+'-view.js',content,()=>{
...
exports.insert2PageForWeweb=function insert2PageForWeweb(page,content){
  ...
  if(cmpts){
    content += '@code-separator-line:'+JSON.stringify(cmpts)// components array
