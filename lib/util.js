...
exports.parseImports = function parseImports (file, wxss, cb) {
  let fn = wxss ? 'parseCssImportsSync' : 'parseImportsWx'
  let srcs = []
  this[fn](srcs, file, function (err) {
    if (err) {
...
exports.parseImportsWx = function parseImportsWx (res, file, cb) {
...
// function parseCssImports (res, file, cb) {
//   let re = /\s*@import\s+[^;]+?;/g
//   fs.readFile(file, 'utf8', (err, content) => {
...
exports.parseJavascript = function (config, full_path,needbabel,isModuleP) {
  return new Promise(function (resolve, reject) {
    let babel=false
    if(needbabel==undefined){
      babel=config.babel
    }else{
      babel=needbabel
    }
    let isModule =false
    if(isModuleP==undefined){
      isModule =
      full_path != 'app.js' &&
    ...
    loadJavascript(full_path, babel, function (err, result) {
      if (err) return reject(err)
